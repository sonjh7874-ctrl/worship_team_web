...
Phase 0(기반 구축)·Phase 1(콘티 기능 마무리)·Phase 2(공지사항 + 월간 스케줄 기본 CRUD)·Phase 3(메인 대시보드)까지 완료. 콘티/공지사항/인명부/월간 스케줄(배정 포함)/대시보드 전부 화면-API-DB 흐름이 동작하고, 실사용 테스트 중 발견된 버그 6건(배정 편집 시 미리채우기, 콰이어 팀 제한, 팀원 삭제 500, 중복생성 500, draft 콘티 노출, 페이지 간 네비게이션 누락)도 수정 완료. 자세한 내용은 `docs/전체_로드맵.md`의 Phase 1~3 절 참고.

다음은 **Phase 4 — 싱어팀 마이크 배치 시각화**:

- [ ] 무대 좌표(마이크 1~8, 콰이어 위치)를 프론트 상수로 고정
- [ ] 데이터만 갈아끼우는 CSS Grid 컴포넌트 구현
- [ ] 스케줄 화면(및 필요하면 대시보드)에 컴포넌트 배치
- Phase 2의 스케줄 CRUD 위에 얹는 컴포넌트라 백엔드 변경 불필요

착수 전에 Phase 1~3 때처럼 작업을 더 작은 단위로 쪼갠 설계(SDD)부터 `docs/전체_로드맵.md`에 정리할 것.
