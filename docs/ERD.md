...
- 저작권 있는 콘텐츠라 **가사 관련 조회(`GET /songs/{id}/sections`, `GET /contis/{id}/lyrics`)는 로그인(member 이상)을 요구**한다. 또한 `members`는 이름·성별·생년월일 등 개인정보를 포함하므로 `GET /members`도 member 이상으로 제한한다. 참/불참과 자동 배정 제안은 개인 사유를 포함하거나 그 데이터에서 파생되므로 leader 이상만 조회하며, 나머지 일반 콘텐츠 조회는 비로그인 공개를 유지한다.
